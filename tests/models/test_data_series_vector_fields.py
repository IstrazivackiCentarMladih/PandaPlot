...
def test_data_series_vector_style_column_fields_have_defaults():
    series = DataSeries(dataset_id="ds1", series_type=SeriesType.VECTOR)
    assert series.style.u_column_id == ""
    assert series.style.v_column_id == ""
...
def test_chart_to_dict_from_dict_round_trips_vector_fields():
    ...
    assert series.style.u_column_id == "u"
    assert series.style.v_column_id == "v"
    assert series.style.magnitude_column_id == "m"
...
def test_assign_series_column_ids_fills_u_and_v():
    ...
    series = DataSeries(
        ...
        style=VectorSeriesStyle(u_column="u_name", v_column="v_name"),
    )
    assign_series_column_ids(series, _FakeDataset())
    assert series.style.u_column_id == "u-id"
    assert series.style.v_column_id == "v-id"
...
def test_assign_series_column_ids_fills_magnitude():
    ...
    series = DataSeries(
        dataset_id="ds1", series_type=SeriesType.VECTOR,
        style=VectorSeriesStyle(magnitude_column="mag_name"),
    )
    assign_series_column_ids(series, _FakeDataset())
    assert series.style.magnitude_column_id == "mag-id"
