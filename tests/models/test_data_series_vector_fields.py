...
def test_data_series_vector_fields_have_defaults():
    series = DataSeries(dataset_id="ds1")
    assert series.u_column_id == ""
    assert series.v_column_id == ""
    assert series.magnitude_column_id == ""
    assert series.vector_color == "#1f77b4"
    assert series.vector_colormap == ""
    assert series.vector_scale == 0.0
    assert series.vector_width == 0.005
    assert series.vector_head_width == 3.0
    assert series.vector_head_length == 5.0
    assert series.vector_head_axis_length == 4.5
...
def test_chart_to_dict_from_dict_round_trips_vector_fields():
    ...
    assert series.vector_color == "#ff0000"
    assert series.vector_colormap == "plasma"
    assert series.vector_scale == 2.5
    assert series.vector_width == 0.01
    assert series.vector_head_width == 4.0
    assert series.vector_head_length == 6.0
    assert series.vector_head_axis_length == 5.5


def test_chart_from_dict_defaults_vector_fields_for_legacy_charts():
    """A chart saved before this feature existed has no vector keys at all."""
    chart = Chart(name="Legacy", chart_type="line")
    chart.add_data_series("ds1", x_column_id="x", y_column_id="y")
    data = chart.to_dict()
    for key in ("u_column_id", "v_column_id", "magnitude_column_id", "vector_color",
                "vector_colormap", "vector_scale", "vector_width",
