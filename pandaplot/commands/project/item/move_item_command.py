...
class MoveItemCommand(Command):
    ...
    def execute(self):
        ...
        try:
            # Debug logging
            print(f"MoveItemCommand: Moving item '{self.item_id}' from '{self.source_folder_id}' to '{self.target_folder_id}'")
            ...
            print(f"MoveItemCommand: Moving item '{item_name}' (ID: {self.item_id})")
            
            # Validate target folder exists (if specified)
            if self.target_folder_id and self.target_folder_id != 'root':
                print(f"MoveItemCommand: Looking for target folder '{self.target_folder_id}'")
                ...
                if target_folder is None:
                    ...
                    print(f"Available items in project: {list(project.items_index.keys())}")
                    self.ui_controller.show_error_message(
                        "Move Item Error",
                    ...
                    print(f"MoveItemCommand: Found target folder: {target_folder.name} (type: {type(target_folder).__name__})")
            ...
            print(f"MoveItemCommand: Moved {self.item_type} '{item_name}' (ID: {self.item_id}) from '{self.source_folder_id}' to '{self.target_folder_id}'")
            ...
            if self.move_performed and self.app_state.has_project:
                ...
                if project and self.item_id:
                    ...
                    if item is not None:
                        ...
                        print(f"MoveItemCommand: Undid move of {self.item_type} '{item_name}' (ID: {self.item_id})")
