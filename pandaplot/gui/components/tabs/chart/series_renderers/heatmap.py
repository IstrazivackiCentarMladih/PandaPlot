...
def render_heatmap_series(axes, series_data: SeriesData, style: HeatmapSeriesStyle,
    ...
    # nothing and only produces a "Legend does not support handles for
    # QuadMesh instances" UserWarning on every render. chart_editor.py
    # separately skips axes.legend() entirely when there are no real
    # legend handles, rather than trying to manufacture one here.
    return axes.pcolormesh(
        xs, ys, grid, cmap=extra["colormap"], vmin=vmin, vmax=vmax,
